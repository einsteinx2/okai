...
public:
	...
	void add(_Type element){
		if (items == 0) {
			items = (_Type*)malloc((_length = _MinLen) * sizeof(_Type));
		} else {
			if (length >= _length) {
				_length *= 2;
				items = (_Type*)realloc(items, _length * sizeof(_Type));
			}
		}
		items[length++] = element;
	}

	void insert(_Type element, int i){
		if (items == 0) {
			items = (_Type*)malloc((_length = _MinLen) * sizeof(_Type));
		} else {
			if (length >= _length) {
		...
		if (i >=0 && i < length){
			memcpy(items[i+1],items[i], (length - i) * sizeof(_Type));
			items[i] = element;
			length++;
		} else {
			add(element);
